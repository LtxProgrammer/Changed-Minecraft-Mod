...
public class BuiltinRepositorySource implements RepositorySource {
    ...
    public BuiltinRepositorySource(String modId) throws IOException, NullPointerException {
        this.modId = modId;
        this.modFile = FMLLoader.getLoadingModList().getModFileById(modId).getFile();
        var file = this.modFile.getFilePath().toFile();
        if (file.isDirectory()) {
            this.isJar = false;
            Files.walk(this.modFile.getFilePath().resolve(DATAPACKS_FOLDER), 1).filter(path -> {
    ...
    public void loadPacks(Consumer<Pack> out, Pack.PackConstructor constructor) {
        for(String id : packIds) {
            Pack pack = Pack.create(modId + "/" + id, false,
                    this.createSupplier(this.modFile.getFilePath().toFile(), id), constructor, Pack.Position.TOP, PackSource.BUILT_IN);
            if (pack != null) {
                out.accept(pack);
            }
        }

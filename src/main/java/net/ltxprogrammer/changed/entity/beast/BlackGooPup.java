...
public class BlackGooPup extends AbstractBlackGooEntity {
    protected static final int MAX_AGE = 72000;
    protected int age = 0;
    protected int ticksLeftAsPuddle = 0;
    private static final EntityDataAccessor<Boolean> DATA_PUDDLE_ID = SynchedEntityData.defineId(BlackGooPup.class, EntityDataSerializers.BOOLEAN);

    private final PathNavigation goodNavigator;
    private final DudNavigator dudNavigator;

    public BlackGooPup(EntityType<? extends BlackGooPup> type, Level level) {
        super(type, level);
    ...
    public void visualTick(Level level) {
        ...
        if (age > MAX_AGE) {
            ...
            if (wolf != null) {
                ...
                ChangedSounds.broadcastSound(this, newVariant.sound, 1.0f, 1.0f);
                applyCustomizeToAged((AbstractBlackGooEntity)wolf);
            }
            this.discard();
        }
    }

    protected void applyCustomizeToAged(AbstractBlackGooEntity aged) {
        ...
        if (this.level.isClientSide) {
            ...
            if (!this.isTame() && this.isTameItem(itemstack)) {
                ...
                ProcessTransfur.ifPlayerTransfurred(player, variant -> {
                    if (variant.getGooType() == GooType.BLACK_GOO && this.random.nextInt(3) == 0) { // One in 3 chance
                        this.tame(player);
                        this.navigation.stop();
                        this.setTarget(null);
                        this.level.broadcastEntityEvent(this, (byte)7);
                    } else if (!variant.getGooType().isHostileTo(GooType.BLACK_GOO) && this.random.nextInt(10) == 0) {
                        this.tame(player);
                        this.navigation.stop();
                        this.setTarget(null);
                        this.level.broadcastEntityEvent(this, (byte)7);

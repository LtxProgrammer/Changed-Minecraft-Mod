...
public abstract class PureWhiteGooEntity extends WhiteGooWolfMale {
    public PureWhiteGooEntity(EntityType<? extends PureWhiteGooEntity> p_19870_, Level p_19871_) {
    ...
    protected void registerGoals() {
        super.registerGoals();
        this.targetSelector.addGoal(1, (new HurtByTargetGoal(this, PureWhiteGooEntity.class)));
    }
    ...
    public static final Predicate<PureWhiteGooEntity> IS_STANDING_ON_WHITE_LATEX = pureWhiteGooEntity -> {
        if (!pureWhiteGooEntity.isOnGround())
            return false;
        BlockState standingOn = pureWhiteGooEntity.level.getBlockState(pureWhiteGooEntity.blockPosition().below());
        if (standingOn.is(ChangedBlocks.WHITE_LATEX_BLOCK.get()))
            return true;
        if (standingOn.getProperties().contains(COVERED))
            return standingOn.getValue(COVERED) == GooType.PURE_WHITE_GOO;
        return false;
    };
    ...
    public void tick() {
        ...
        var entities = this.level.getEntitiesOfClass(PureWhiteGooEntity.class, new AABB(blockPosition()).inflate(2.0), IS_STANDING_ON_WHITE_LATEX);
        if (entities.size() <= 12)
            return;

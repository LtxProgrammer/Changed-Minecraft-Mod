...
public class SyncTransfurProgressPacket implements ChangedPacket {
    ...
    public SyncTransfurProgressPacket(FriendlyByteBuf buffer) {
        ...
        this.progress = new ProcessTransfur.TransfurProgress(buffer.readFloat(), ChangedRegistry.TRANSFUR_VARIANT.get().getValue(buffer.readInt()));
    }

    public void write(FriendlyByteBuf buffer) {
        buffer.writeUUID(uuid);
        buffer.writeFloat(progress.progress());
        buffer.writeInt(ChangedRegistry.TRANSFUR_VARIANT.get().getID(progress.variant()));
    }

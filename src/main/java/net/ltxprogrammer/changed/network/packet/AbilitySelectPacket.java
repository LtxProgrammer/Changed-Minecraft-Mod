...
public class AbilitySelectPacket implements ChangedPacket {
    final int id;
    final KeyReference key;
    final AbstractAbility<?> ability;

    public AbilitySelectPacket(Player player, KeyReference key, AbstractAbility<?> ability) {
        this.id = player.getId();
        this.key = key;
        this.ability = ability;
    }

    public AbilitySelectPacket(FriendlyByteBuf buffer) {
        ...
        if (context.getDirection().getReceptionSide() == LogicalSide.CLIENT) {
            ...
            return levelFuture.thenAccept(level -> {
                ProcessTransfur.ifPlayerTransfurred(EntityUtil.playerOrNull(level.getEntity(this.id)), (player, variant) -> {
                    ...
                    if (ability != null)
                        variant.setSelectedAbility(key, ability);
                });
            });
        }

...
public class UpperBodyAttackAnimator<T extends ChangedEntity, M extends AdvancedHumanoidModel<T>> extends AbstractUpperBodyAnimator<T, M> {
    public UpperBodyAttackAnimator(ModelPart head, ModelPart torso, ModelPart leftArm, ModelPart rightArm) {
    ...
    protected void setupAttackAnimation(T entity, float ageInTicks) {
        ...
        if (!(core.entityModel.attackTime <= 0.0F)) {
            ...
            modelpart.yRot += torso.yRot * 2.0F;
            modelpart.zRot += Mth.sin(core.entityModel.attackTime * (float)Math.PI) * -0.4F;
        }
    }
}

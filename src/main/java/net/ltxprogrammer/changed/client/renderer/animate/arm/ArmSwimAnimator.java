...
public class ArmSwimAnimator<T extends ChangedEntity, M extends EntityModel<T>> extends HumanoidAnimator.Animator<T, M> {
    ...
    public void setupAnim(@NotNull T entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        if (!entity.isUsingItem()) {
            ...
            if (f5 < 14.0F) {
                leftArm.xRot = HumanoidAnimator.rotlerpRad(f2, leftArm.xRot, 0.0F);
                rightArm.xRot = Mth.lerp(f1, rightArm.xRot, 0.0F);
                leftArm.yRot = HumanoidAnimator.rotlerpRad(f2, leftArm.yRot, (float)Math.PI);
                rightArm.yRot = Mth.lerp(f1, rightArm.yRot, (float)Math.PI);
                leftArm.zRot = HumanoidAnimator.rotlerpRad(f2, leftArm.zRot, (float)Math.PI + 1.8707964F * this.quadraticArmUpdate(f5) / this.quadraticArmUpdate(14.0F));
                ...
                leftArm.xRot = HumanoidAnimator.rotlerpRad(f2, leftArm.xRot, ((float)Math.PI / 2F) * f6);
                rightArm.xRot = Mth.lerp(f1, rightArm.xRot, ((float)Math.PI / 2F) * f6);
                leftArm.yRot = HumanoidAnimator.rotlerpRad(f2, leftArm.yRot, (float)Math.PI);
                rightArm.yRot = Mth.lerp(f1, rightArm.yRot, (float)Math.PI);
                leftArm.zRot = HumanoidAnimator.rotlerpRad(f2, leftArm.zRot, 5.012389F - 1.8707964F * f6);
                rightArm.zRot = Mth.lerp(f1, rightArm.zRot, 1.2707963F + 1.8707964F * f6);
            } else if (f5 >= 22.0F && f5 < 26.0F) {
                float f3 = (f5 - 22.0F) / 4.0F;
                leftArm.xRot = HumanoidAnimator.rotlerpRad(f2, leftArm.xRot, ((float)Math.PI / 2F) - ((float)Math.PI / 2F) * f3);
                rightArm.xRot = Mth.lerp(f1, rightArm.xRot, ((float)Math.PI / 2F) - ((float)Math.PI / 2F) * f3);
                leftArm.yRot = HumanoidAnimator.rotlerpRad(f2, leftArm.yRot, (float)Math.PI);
                rightArm.yRot = Mth.lerp(f1, rightArm.yRot, (float)Math.PI);
                leftArm.zRot = HumanoidAnimator.rotlerpRad(f2, leftArm.zRot, (float)Math.PI);

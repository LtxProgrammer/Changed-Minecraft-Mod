...
public class WindGustRenderer extends EntityRenderer<WindGust> {
    public WindGustRenderer(EntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    public void render(WindGust particle, float p_114486_, float partialTick, PoseStack pose, MultiBufferSource buffers, int packedLight) {
        if (partialTick < 0.6f)
            return;

        float dispersion = (((float)particle.tickCount / (float)WindGust.DISSIPATE_TIME) * 3.0f) + 0.001f;
        ...
        particle.level().addParticle(ParticleTypes.CLOUD,
                particle.xo + dx * 1.2, particle.yo + dh, particle.zo + dz * 1.2,
                delta.x * 0.75f, delta.y * 0.75f, delta.z * 0.75f);
    }

    @Override
    public @NotNull ResourceLocation getTextureLocation(WindGust particle) {

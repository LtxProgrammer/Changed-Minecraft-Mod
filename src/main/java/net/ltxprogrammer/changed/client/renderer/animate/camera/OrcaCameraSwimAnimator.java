...
public class OrcaCameraSwimAnimator<T extends LatexEntity, M extends EntityModel<T>> extends LatexAnimator.CameraAnimator<T, M> {
    @Override
    ...
    public void setupAnim(@NotNull CameraExtender camera, @NotNull T entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        Vec3 upOffset = camera.getUpDirection().multiply(computedOffset, computedOffset, computedOffset);

        camera.setCameraPosition(camera.getCameraPosition().add(upOffset));
    }
}

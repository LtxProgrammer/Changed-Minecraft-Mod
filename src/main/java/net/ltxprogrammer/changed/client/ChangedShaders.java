...
public class ChangedShaders {
    ...
    @Nullable
    private static ShaderInstance rendertypeWaveVisionEntityResonant;
    @Nullable
    ...
    public static ShaderInstance getWaveVisionEntityResonantShader() {
        return Objects.requireNonNull(rendertypeWaveVisionEntityResonant, "Attempted to call getWaveVisionEntityResonantShader before shaders have finished loading.");
    }
    ...
    public static void reloadShaders(ResourceManager resourceManager, Consumer<Pair<ShaderInstance, Consumer<ShaderInstance>>> loader) throws IOException {
        ...
        loader.accept(Pair.of(new ShaderInstance(resourceManager, Changed.modResource("rendertype_wave_vision_entity_resonant"), DefaultVertexFormat.NEW_ENTITY), (instance) -> {
            rendertypeWaveVisionEntityResonant = instance;
        }));
    ...
    protected static final Function<Vector3f, RenderStateShard.ShaderStateShard> RENDERTYPE_WAVE_VISION_ENTITY_RESONANT_SHADER = Util.memoize(resonance -> {
        return new PrefixedShaderShard(ChangedShaders::getWaveVisionEntityResonantShader,
    ...
    private static final BiFunction<ResourceLocation, Vector3f, RenderType> WAVE_VISION_RESONANT = Util.memoize((texture, resonance) -> {
        RenderType.CompositeState rendertype$compositestate = RenderType.CompositeState.builder()
                .setShaderState(RENDERTYPE_WAVE_VISION_ENTITY_RESONANT_SHADER.apply(resonance))
                .setTextureState(new RenderStateShard.TextureStateShard(texture, false, false))
                .setTransparencyState(NO_TRANSPARENCY)
                .setLightmapState(LIGHTMAP)
                .setOverlayState(OVERLAY)
                .setCullState(NO_CULL)
                .createCompositeState(true);
        return RenderType.create(Changed.modResourceStr("wave_vision_entity_resonant"), DefaultVertexFormat.NEW_ENTITY, VertexFormat.Mode.QUADS, 256, true, false, rendertype$compositestate);
    });

    private static final Function<RenderStateShard.TextureStateShard, RenderType> WAVE_VISION_SOLID = Util.memoize((textureStateShard) ->
            RenderType.create(Changed.modResourceStr("wave_vision_solid"), DefaultVertexFormat.BLOCK, VertexFormat.Mode.QUADS, 2097152, true, false, RenderType.CompositeState.builder()
                    ...
                    .createCompositeState(true)));
    private static final Function<RenderStateShard.TextureStateShard, RenderType> WAVE_VISION_CUTOUT_MIPPED = Util.memoize((textureStateShard) ->
            RenderType.create(Changed.modResourceStr("wave_vision_cutout_mipped"), DefaultVertexFormat.BLOCK, VertexFormat.Mode.QUADS, 2097152, true, false, RenderType.CompositeState.builder()
                    ...
                    .setTextureState(textureStateShard)
                    .createCompositeState(true)));
    private static final Function<RenderStateShard.TextureStateShard, RenderType> WAVE_VISION_CUTOUT = Util.memoize((textureStateShard) ->
